...
export async function DELETE(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await db.analisisKebutuhan.delete({ where: { id } })
    await auditLog(session, 'DELETE_PERMANENT', 'ARSIP_ANALISIS', `Hapus permanen analisis: ${item.judul}`, req)
    return NextResponse.json({ success: true })
  } catch (e) {
    console.error('arsip analisis delete permanent error:', e)
