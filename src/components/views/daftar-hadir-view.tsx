...
function formatDateHeader(dateStr: string): { day: string; date: string } {
  ...
  return { day: dt.toLocaleDateString('id-ID', { weekday: 'short' }), date: String(dt.getDate()) }
}
...
export function DaftarHadirView() {
  ...
  // Filter dates by selected days — if none selected, show all
  const dates = useMemo(() => {
    if (selectedDays.size === 0) return allDates
    return allDates.filter((d) => {
      const dt = new Date(d + 'T00:00:00')
      return selectedDays.has(dt.getDay())
    })
  }, [allDates, selectedDays])

  function toggleDay(dayKey: number) {
  ...
  useEffect(() => {
    if (selectedAngkatan) { fetchMatrix(selectedAngkatan) }
    else { setPesertaList([]); setKehadiranMap({}); setAllDates([]); setSelectedAngkatanData(null) }
  ...
  function handleCellClick(pesertaId: string, tanggal: string) {
    ...
    setKehadiranMap((prev) => {
      ...
      const nextMap = { ...prev }
      if (next) nextMap[key] = next; else delete nextMap[key]
      return nextMap
    })
  }
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (res.ok) { setHasChanges(false); const data = await res.json(); toast({ title: 'Berhasil', description: `Kehadiran tersimpan (${data.count} data)` }) }
      else { toast({ title: 'Gagal', description: 'Gagal menyimpan kehadiran', variant: 'destructive' }) }
    } catch { toast({ title: 'Gagal', description: 'Gagal menyimpan kehadiran', variant: 'destructive' }) }
    setSaving(false)
  }

  function handleFillAll() {
    setHasChanges(true)
    const newMap: Record<string, string> = { ...kehadiranMap }
    for (const p of pesertaList) { for (const d of dates) { newMap[`${p.pesertaId}_${d}`] = 'HADIR' } }
  ...
  function handleClearAll() {
    setHasChanges(true)
    const newMap: Record<string, string> = { ...kehadiranMap }
    for (const p of pesertaList) { for (const d of dates) { delete newMap[`${p.pesertaId}_${d}`] } }
    setKehadiranMap(newMap)
  }

  async function handleExport(format: 'pdf' | 'xls') {
    if (!selectedAngkatan) { toast({ title: 'Perhatian', description: 'Pilih angkatan terlebih dahulu', variant: 'destructive' }); return }
    setLoadingExport(format)
    try {
      let url = `/api/daftar-hadir/export/${format}?angkatanId=${selectedAngkatan}`
      if (selectedDays.size > 0) url += `&days=${Array.from(selectedDays).sort().join(',')}`
      ...
      a.href = blobUrl
      a.download = format === 'pdf' ? `rekap-kehadiran-${selectedAngkatanData?.namaPelatihan || 'peserta'}.pdf` : `rekap-kehadiran-${selectedAngkatanData?.namaPelatihan || 'peserta'}.xlsx`
    ...
    } catch { toast({ title: 'Gagal', description: `Gagal export ${format.toUpperCase()}`, variant: 'destructive' }) }
    setLoadingExport(null)
  }

  function getStats() {
    let h = 0, s = 0, i = 0, a = 0, empty = 0
    for (const p of pesertaList) { for (const d of dates) { const st = kehadiranMap[`${p.pesertaId}_${d}`]; if (st === 'HADIR') h++; else if (st === 'SAKIT') s++; else if (st === 'IZIN') i++; else if (st === 'ALPA') a++; else empty++ } }
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Rekap Kehadiran Peserta" description="Isi matrix kehadiran, lalu export ke PDF atau Excel" />
      <Card>
        <CardHeader className="pb-4">
          <CardTitle className="text-base font-semibold flex items-center gap-2"><FilterIcon className="w-4 h-4" /> Pilih Angkatan Pelatihan</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <div>
            ...
            <Select value={selectedAngkatan} onValueChange={setSelectedAngkatan}>
              <SelectTrigger className="w-full"><SelectValue placeholder="-- Pilih Angkatan --" /></SelectTrigger>
              <SelectContent>
                {angkatanOptions.map((a) => (<SelectItem key={a.id} value={a.id}><span className="line-clamp-1">{a.label}</span></SelectItem>))}
              </SelectContent>
            </Select>
          </div>
          ...
          <CardHeader className="pb-3">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
              <CardTitle className="text-base font-semibold flex items-center gap-2">
                ...
                Matrix Kehadiran
                {dates.length > 0 && <Badge variant="secondary" className="text-xs">{dates.length} hari</Badge>}
              </CardTitle>
              <div className="flex flex-wrap gap-2">
                <Button size="sm" variant="outline" onClick={handleFillAll} disabled={saving || pesertaList.length === 0}><Users className="w-3.5 h-3.5 mr-1" /> Semua Hadir</Button>
                <Button size="sm" variant="outline" onClick={handleClearAll} disabled={saving || pesertaList.length === 0}><RotateCcw className="w-3.5 h-3.5 mr-1" /> Reset</Button>
              </div>
            </div>
          </CardHeader>
          <CardContent>
            {loading ? (
              <div className="flex items-center justify-center py-16"><Loader2 className="w-6 h-6 animate-spin text-[#0F4C81]" /></div>
            ) : pesertaList.length === 0 ? (
              <div className="text-center py-16 text-slate-400"><Users className="w-12 h-12 mx-auto mb-3 opacity-40" /><p className="text-sm">Tidak ada peserta pada angkatan ini</p></div>
            ) : (
              <>
                {/* Day Filter */}
                <div className="mb-4">
                  <label className="text-xs font-medium text-slate-500 mb-2 block">Filter Hari (klik untuk pilih, kosongkan = semua hari)</label>
                  <div className="flex flex-wrap gap-1.5">
                    {DAY_OPTIONS.map((day) => {
                      const active = selectedDays.has(day.key)
                      return (
                        <button key={day.key} type="button" onClick={() => toggleDay(day.key)}
                          className={`px-3 py-1.5 rounded-lg text-xs font-semibold border transition-all ${active ? 'bg-[#0F4C81] text-white border-[#0F4C81] shadow-sm' : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-100 hover:border-slate-300'}`}>
                          {day.label}
                        </button>
                      )
                    })}
                    {selectedDays.size > 0 && (
                      <button type="button" onClick={() => setSelectedDays(new Set())} className="px-3 py-1.5 rounded-lg text-xs font-medium text-red-600 hover:bg-red-50 border border-transparent">✕ Reset Filter</button>
                    )}
                  </div>
                </div>
                  ...
                  <table className="w-full border-collapse text-xs min-w-[600px]">
                    <thead className="sticky top-0 z-10">
                      <tr className="bg-[#0F4C81] text-white">
                        ...
                        {dates.map((d) => { const { day, date } = formatDateHeader(d); return (<th key={d} className="px-1 py-2 text-center min-w-[36px] border-r border-white/20 last:border-r-0"><div className="font-normal text-[10px] opacity-80">{day}</div><div className="font-bold text-sm">{date}</div></th>) })}
                      </tr>
                    </thead>
                    <tbody>
                      {pesertaList.map((p, rowIdx) => (
                        <tr key={p.pesertaId} className={rowIdx % 2 === 0 ? 'bg-white' : 'bg-slate-50/60'}>
                          <td className="px-2 py-1.5 text-center text-slate-500 border-r border-slate-100 font-medium">{p.no}</td>
                          <td className="px-2 py-1.5 text-left border-r border-slate-100 font-medium text-slate-800"><div className="line-clamp-1">{p.nama}</div></td>
                          <td className="px-2 py-1.5 text-left border-r border-slate-100 text-slate-500 font-mono">{p.nip}</td>
                          {dates.map((d) => {
                            ...
                            return (<td key={d} className="px-0.5 py-0.5"><button type="button" onClick={() => handleCellClick(p.pesertaId, d)} className={`w-full h-8 rounded text-xs font-bold border transition-colors ${colorClass}`} title={`${p.nama} — ${formatDateShort(d)}: ${status || 'Belum diisi'}`}>{shortCode || '—'}</button></td>)
                          })}
                        </tr>
                      ))}
                    </tbody>
                ...
                {stats && (
                  <div className="flex flex-wrap items-center gap-4 mt-4 text-xs text-slate-600">
                    ...
                    {hasChanges && <span className="ml-auto text-amber-600 font-medium animate-pulse">Ada perubahan belum disimpan</span>}
                  </div>
                )}

                <div className="flex flex-wrap gap-3 mt-5 pt-4 border-t border-slate-200">
                  <Button onClick={handleSave} disabled={!hasChanges || saving} className="bg-[#195737] hover:bg-[#0F4227] text-white gap-2">
                    {saving ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />}{hasChanges ? 'Simpan Kehadiran' : 'Tersimpan'}
                  </Button>
                  <div className="flex-1" />
                  <Button onClick={() => handleExport('pdf')} disabled={loadingExport !== null} className="bg-[#0F4C81] hover:bg-[#0a3a63] text-white gap-2">
                    {loadingExport === 'pdf' ? <Loader2 className="w-4 h-4 animate-spin" /> : <FileText className="w-4 h-4" />}Export PDF
                  </Button>
                  <Button onClick={() => handleExport('xls')} disabled={loadingExport !== null} variant="outline" className="gap-2 border-[#0F4C81] text-[#0F4C81] hover:bg-[#0F4C81]/10">
                    {loadingExport === 'xls' ? <Loader2 className="w-4 h-4 animate-spin" /> : <FileSpreadsheet className="w-4 h-4" />}Export Excel
                  </Button>
                </div>
              </>
            )}
...
function InfoItem({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return (<div className="flex items-start gap-2.5"><div className="mt-0.5 text-[#195737]">{icon}</div><div><p className="text-xs text-slate-500">{label}</p><p className="text-sm font-medium text-slate-800">{value}</p></div></div>)
}
function LegendItem({ code, label, color }: { code: string; label: string; color: string }) {
  return (<span className="flex items-center gap-1.5"><span className={`w-3 h-3 rounded ${color}`} /><span>{code} = {label}</span></span>)
}
function StatBadge({ label, count, color }: { label: string; count: number; color: string }) {
  if (count === 0) return null
  return (<span className={`px-2 py-0.5 rounded-full font-medium ${color}`}>{label}: {count}</span>)
}
