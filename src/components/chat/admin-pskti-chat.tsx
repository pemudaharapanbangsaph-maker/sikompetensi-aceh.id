...
const WELCOME_MESSAGE: ChatMessage = {
  role: 'assistant',
  content:
    'Halo! 👋 Selamat datang di SIKOMPETENSI ACEH. Saya **Admin PSKTI**, asisten virtual yang siap membantu Anda seputar pendaftaran pelatihan, cek status, program pelatihan, dan informasi lainnya.\n\n🛡️ **Penting:** Demi keamanan data Anda, mohon **JANGAN** mengetik data sensitif (NIP, NPWP, nomor rekening, password) di chat ini. Data tersebut cukup diisi di form resmi.\n\nAda yang bisa saya bantu hari ini? 😊',
  timestamp: Date.now(),
}
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            ...
            <div className="bg-amber-50 border-b border-amber-200 px-3 py-2 flex items-start gap-2 flex-shrink-0">
              <ShieldAlert className="w-3.5 h-3.5 text-amber-600 flex-shrink-0 mt-0.5" />
              <p className="text-[10px] text-amber-700 leading-relaxed">
                <strong>Jangan ketik data sensitif</strong> (NIP, NPWP, rekening,
                password) di chat ini. Gunakan form resmi untuk data tersebut.
              </p>
            </div>
                    ...
                    {msg.role === 'assistant' && (
                      <div
                        className={`flex-shrink-0 w-7 h-7 rounded-full flex items-center justify-center mt-0.5 ${
                          msg.blocked
                            ? 'bg-amber-500'
                            : 'bg-gradient-to-br from-[#195737] to-[#0F4C81]'
                        }`}
                      >
                        {msg.blocked ? (
            ...
            <div className="bg-slate-50 border-t border-slate-100 px-4 py-1.5 text-center flex-shrink-0">
              <p className="text-[10px] text-slate-400">
                Powered by Google Gemini AI • BPSDM Aceh - Bidang PSKTI
              </p>
            </div>
          </motion.div>
        )}
